...
const UsedProductListPage = ({ ads, models, colors, cities }: any) => {
  ...
  const filteredArr = dataFilter?.previewData?.reduce((acc, current) => {
    const x = acc.find((item: any) => item.advertiser_id === current.advertiser_id);
    if (!x) {
...
export default UsedProductListPage;
